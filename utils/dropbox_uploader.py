            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(token_url, data=data, headers={
                    ...
                    if resp.status != 200:
                        ...
                        logger.info(f"Dropbox token refresh failed: {resp.status} {text[:200]}")
                        return None
...
class DropboxUploader:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.post(api_url, json=payload, headers={
                ...
                if resp.status in (200, 409):
                    ...
                    return True
                text = await resp.text()
                logger.info(f"Dropbox ensure_folder failed for {path}: {resp.status} {text[:200]}")
                return False
    ...
    async def upload_file(local_path: str, dropbox_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Dropbox upload read error for {local_path}: {e}")
            return False
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(content_url, data=data, headers={
                ...
                if resp.status == 200:
                    return True
                text = await resp.text()
                logger.info(f"Dropbox upload failed for {dropbox_path}: {resp.status} {text[:200]}")
                return False
